...
![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20Linux-ff1493?style=for-the-badge)
![License](https://img.shields.io/badge/license-MIT-e91e8c?style=for-the-badge)
![Version](https://img.shields.io/badge/version-v1.6.0-ff69b4?style=for-the-badge)
...
[Download .exe](https://github.com/kl6969/stormworks-udp-proxy/releases/latest) · [Download .py](https://github.com/kl6969/stormworks-udp-proxy/releases/latest) · [Report a bug](https://github.com/kl6969/stormworks-udp-proxy/issues)
...
- [Building from Source](#building-a-standalone-executable)
- [How it Works](#how-it-works-in-detail)
...
When Steam picks **WebSockets**, the server still appears in the server browser and looks perfectly fine — but **players cannot join via the server browser**. The only working connection methods are Steam friends list and LAN. When Steam picks **UDP**, server browser joining works normally.

This makes the server randomly unjoinable with no obvious cause, since the server appears completely healthy.
...
> ⚠️ **Cloud VM users (AWS, GCP, DigitalOcean, etc.)**
>
> If your VM uses cloud-init to manage `/etc/hosts`, the patch will be wiped on reboot. To make it permanent:
> ```bash
...
6. For `/ISteamDirectory/GetCMListForConnect`, it strips all `"type" "websockets"` entries from the VDF response
...
Stale entry in hosts file from a previous crashed run.
```cmd
# Windows
powershell -Command "(Get-Content C:\Windows\System32\drivers\etc\hosts) -notmatch 'steampowered' | Set-Content C:\Windows\System32\drivers\etc\hosts"
```
```bash
# Linux
sudo sed -i '/steampowered/d' /etc/hosts
```

**`web API call failed (status = 502)` in the connection log**

The proxy isn't intercepting — check it's running and the cert is installed.
